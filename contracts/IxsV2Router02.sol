...
contract IxsV2Router02 is IIxsV2Router02 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = IxsV2Library.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = IxsV2Library.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'IxsV2Router: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = IxsV2Library.quote(amountBDesired, reserveB, reserveA);
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IIxsV2Pair(pair).burn(to);
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, IxsV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = IxsV2Library.sortTokens(input, output);
            IIxsV2Pair pair = IIxsV2Pair(IxsV2Library.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, IxsV2Library.pairFor(factory, path[0], path[1]), amountIn
        );
